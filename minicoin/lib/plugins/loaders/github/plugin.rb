module Minicoin
    module GithubLoader
        class Plugin < Vagrant.plugin("2")
            name "Minicoin loader for GitHub machines"
            ...
            def self.load_minicoin()
                return {} unless (ENV["MINICOIN_EXPERIMENTAL"] || "").include?("github")
                return {} if ENV['MINICOIN_PROJECT_DIR'].nil?

                path = File.dirname(__FILE__)
                github = YAML.load_file(File.join(path, "github.yml"))
                ...
                while github_root
                    ...
                    jobs.each do |jobname, job|
                        ...
                        matrix_includes.each do |matrix_entry|
                            minicoin_box = github[matrix_entry["os"]]
                            if minicoin_box.nil?
                        ...
                        github_machines.each do |github_machine|
                            ...
                            github_machine["jobs"] += [
                                ...
                                }.merge(github_machine["_github"]["runner"] || {})
                            ]
