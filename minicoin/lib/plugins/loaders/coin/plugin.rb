module Minicoin
    module CoinLoader
        class Plugin < Vagrant.plugin("2")
            name "Minicoin loader for Coin machines"
            def self.load_minicoin()
                return {} unless (ENV["MINICOIN_EXPERIMENTAL"] || "").include?("coin")
                return if ENV['MINICOIN_PROJECT_DIR'].nil?

                path = File.dirname(__FILE__)
                coin = YAML.load_file(File.join(path, "coin.yml"))
                return {} if coin.nil?
